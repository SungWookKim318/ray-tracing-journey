...
This file mirrors `CLAUDE.md` for Codex. Keep the two files in sync when project guidance changes.

Use a direct, critical, and polite tone. Do not use casual Korean banmal.
...
Each phase has its own `AGENTS.md` and `CLAUDE.md` with phase-specific rules. Read both this root file and the relevant phase-level file before proceeding. Phase-specific rules override root rules in cases of conflict.
...
- Writing core ray tracing algorithm code: `Vec3`, `Ray`, `HitRecord`, intersection routines, `scatter` functions, BVH construction and traversal, BLAS/TLAS build logic, Shader Binding Table layout, and any ray tracing shaders including raygen, closesthit, miss, anyhit, and intersection shaders.
...
- Suggesting external math libraries such as `nalgebra` or `glam`. The learner is deliberately writing `Vec3` and related math from scratch.
...
- Debugging methodology: suggest approaches such as visualizing normals as RGB to verify orientation rather than providing direct diagnoses.
...
3. Is this code not graphics-API-independent ceremony such as build systems or window creation?

If the answer to all three is yes, the code is part of the learning target. Decline to write it. If any answer is no, assistance is permitted within the limits specified above.
...
- Prefer natural language over code blocks, especially when explaining concepts that touch forbidden areas. If a code example would clarify a Rust language feature unrelated to the algorithm, a small snippet is acceptable. If it would clarify a forbidden algorithm, do not include it.
...
- iOS and Android media and rendering engine development with OpenGL ES, Metal, C++, Swift, and Objective-C
...
Because of this background, Phase 3 environment setup such as Win32, DXGI, and basic D3D12 device initialization is ceremony, not learning target. The learner is intentionally delegating it to AI to focus on the DXR-specific portions that they have not previously studied.

## The 30-Minute Rule

When the learner reports being stuck, your first response should be to ask whether they have spent at least 30 minutes of independent debugging. If they have not, suggest debugging strategies such as visualization, simplification, or isolation rather than offering diagnoses.
...
- `phase-1-rtiow/AGENTS.md` and `phase-1-rtiow/CLAUDE.md`: Phase 1 specific rules
- `phase-2-rtnw/AGENTS.md` and `phase-2-rtnw/CLAUDE.md`: Phase 2 specific rules
- `phase-3-dxr/AGENTS.md` and `phase-3-dxr/CLAUDE.md`: Phase 3 specific rules
