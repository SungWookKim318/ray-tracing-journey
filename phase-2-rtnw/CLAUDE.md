...
This directory contains the Phase 2 implementation: Ray Tracing: The Next Week in Rust. The root `CLAUDE.md` applies; this file adds Phase 2 specific constraints.
...
In addition to all Phase 1 prohibitions (which remain in effect for any code extending or modifying the existing tracer):
...
- New geometric primitives: `Quad`, lights (emissive material scatter behavior).
...
- **Debugging methodology questions** for visual artifacts. For example, when Perlin noise output looks incorrect, suggest visualization approaches (rendering noise to a 2D image first, isolating frequency vs amplitude effects) rather than diagnosing the algorithm.
...
- Understanding the construction tradeoffs (split heuristic, build time vs traversal time) by writing one is the only way to develop intuition that transfers to API-driven acceleration structures.
...
If the learner pursues the optional challenge of loading external mesh files, the parser itself is partially ceremony (file format parsing is not a learning target). However:
