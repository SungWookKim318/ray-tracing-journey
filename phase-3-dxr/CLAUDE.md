...
This directory contains the Phase 3 implementation: hardware-accelerated ray tracing using DXR (D3D12 Raytracing) in C++. The root `CLAUDE.md` applies; this file adds Phase 3 specific constraints.
...
Phase 3 differs from Phases 1 and 2 in that it has a clearly demarcated environment setup phase ("Pre-Phase 3") where AI assistance is broadly permitted, followed by the main learning phase where DXR core concepts are strictly off-limits to AI.

Read which sub-phase the learner is in before responding.

## Pre-Phase 3 (Environment Setup): AI Assistance Broadly Permitted

The learner has prior experience with D3D9 (undergraduate game project) and self-studied D3D12. The Win32 window, DXGI swapchain, and basic D3D12 device initialization are not new learning material for them. This pre-phase is explicitly delegated to AI to preserve focus for the DXR-specific portions.

### Fully permitted in Pre-Phase 3

- `CMakeLists.txt` for D3D12 + DXC + Windows SDK integration
- Win32 window code (HWND creation, `WndProc`, message loop, DPI awareness, ALT+ENTER handling)
- DXGI factory, adapter enumeration, swapchain creation
- D3D12 device creation, validation layer enablement
- Command queue, command allocator, command list setup
...
Rather than generating from scratch, suggest the learner copy the structure of Microsoft's official sample [`D3D12HelloWindow`](https://github.com/microsoft/DirectX-Graphics-Samples/tree/master/Samples/Desktop/D3D12HelloWorld/src/HelloWindow). This is known-good baseline code. Assist with adapting it to the learner's project rather than writing equivalent code from scratch.

### What you should still do during Pre-Phase 3
...
- Any DXR-related code: BLAS/TLAS, SBT, RT pipeline state objects, RayQuery, ray tracing shaders.
...
### Permitted (carrying over from Pre-Phase 3)

- `HRESULT` interpretation and validation layer error message analysis.
- DXC compiler invocation, shader compilation troubleshooting (separate from shader content authoring).
...
- DXR API usage patterns. For example, the learner may ask "why does `CreateStateObject` take so many subobjects?" Explain the design rationale in natural language. Do not write the call site.
...
- [Microsoft DirectX-Graphics-Samples — D3D12 Raytracing](https://github.com/microsoft/DirectX-Graphics-Samples/tree/master/Samples/Desktop/D3D12Raytracing) for working code patterns.
- [Chris Wyman — A Gentle Introduction to DXR](https://intro-to-dxr.cwyman.org/) for conceptual scaffolding.
- [Microsoft DirectX Raytracing Specification](https://microsoft.github.io/DirectX-Specs/d3d/Raytracing.html) for authoritative API semantics.
- Ray Tracing Gems I and II (free PDFs) for technique chapters.
...
- Suggest tools (ImageMagick `compare`, perceptual diff tools, simple Python scripts using NumPy).
- Discuss expected sources of small differences (RNG seeding, floating point ordering in BVH traversal).
- Do not write the path tracer or any of its DXR-side counterparts.

## SBT Is the Hardest Part: Reinforce Constraint Here
...
Offer methodology: suggest they start by drawing the SBT layout on paper (raygen record, miss records, hit group records), then translate that diagram into code themselves.

## Inline Ray Tracing vs RT Pipeline

The learner is expected to implement both inline ray tracing (RayQuery) and the full RT pipeline (with SBT) and compare them. Do not let them skip one in favor of the other. Both forms have distinct learning value.
